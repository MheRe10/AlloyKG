...
class LightRAG:
    ...
    async def ainsert(
        ...
        print("ainsert:")
        print("input:", str(input)[:200])
        print("ids:", ids)
        print("file_paths:", file_paths)
        print("track_id:", track_id)
        await self.apipeline_enqueue_documents(input, ids, file_paths, track_id)
        print("apipeline_enqueue_documents done")
        await self.apipeline_process_enqueue_documents(
            split_by_character, split_by_character_only
        )
        print("apipeline_process_enqueue_documents done")
    ...
    async def apipeline_enqueue_documents(
        ...
        print(f"DEBUG: all_new_doc_ids count: {len(all_new_doc_ids)}")
        print(f"DEBUG: all_new_doc_ids: {list(all_new_doc_ids)}")
        
        # Exclude IDs of documents that are already in progress
        unique_new_doc_ids = await self.doc_status.filter_keys(all_new_doc_ids)
        print(f"DEBUG: After filter_keys, unique_new_doc_ids: {list(unique_new_doc_ids)}")
        print(f"DEBUG: This means {len(all_new_doc_ids) - len(unique_new_doc_ids)} documents already exist in doc_status")
        ...
        print(f"DEBUG: unique_new_doc_ids count: {len(unique_new_doc_ids)}")
        print(f"DEBUG: new_docs count after filtering: {len(new_docs)}")
        print(f"DEBUG: new_docs keys: {list(new_docs.keys())}")
        
        if not new_docs:
            print("DEBUG: No new unique documents were found - returning early!")
        ...
        print(f"DEBUG: About to write full_docs_data with {len(full_docs_data)} documents")
        print(f"DEBUG: full_docs storage path: {getattr(self.full_docs, '_file_name', 'No file name')}")
        try:
            await self.full_docs.upsert(full_docs_data)
            print(f"DEBUG: Successfully wrote full_docs_data to memory")
            
            # 立即调用 index_done_callback 来持久化数据
            print(f"DEBUG: Calling index_done_callback to persist data to disk")
            await self.full_docs.index_done_callback()
            print(f"DEBUG: index_done_callback completed - data should be on disk now")
        except Exception as e:
            print(f"ERROR: Failed to write full_docs_data: {e}")
            raise
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # print("*DEBUG* full_docs:", self.full_docs)
                            print("*DEBUG* ID:", doc_id)
                            ...
                            content = content_data["content"]
                            print(f"*DEBUG* Retrieved content of length {len(content)} for doc_id: {doc_id}")

                            # Generate chunks from document
                            print(f"*DEBUG* Starting chunking process...")
                            print(f"*DEBUG* chunk_token_size: {self.chunk_token_size}")
                            print(f"*DEBUG* chunk_overlap_token_size: {self.chunk_overlap_token_size}")
                            ...
                            try:
                                ...
                                print(f"*DEBUG* Chunking completed, got {len(chunking_result)} chunks")
                                ...
                                print(f"*DEBUG* Generated {len(chunks)} chunk dictionaries")
                            ...
                            print(f"*DEBUG* Created doc_status_task for doc_id: {doc_id}")
                            ...
                            print(f"*DEBUG* Created chunks_vdb_task and text_chunks_task for doc_id: {doc_id}")
                            ...
                            print(f"*DEBUG* About to execute first stage tasks: {len(first_stage_tasks)} tasks")
                            print(f"*DEBUG* Tasks: doc_status_task, chunks_vdb_task, text_chunks_task")
                            
                            # Execute first stage tasks
                            try:
                                print(f"*DEBUG* Starting asyncio.gather for first stage tasks...")
                                await asyncio.gather(*first_stage_tasks)
                                print(f"*DEBUG* First stage tasks completed successfully")
                            except Exception as e:
                                print(f"*ERROR* First stage tasks failed: {e}")
                                raise

                            # Stage 2: Process entity relation graph (after text_chunks are saved)
                            print(f"*DEBUG* Starting Stage 2: entity relation graph processing")
                            entity_relation_task = asyncio.create_task(
                                self._process_entity_relation_graph(
                                    chunks, pipeline_status, pipeline_status_lock
                                )
                            )
                            print(f"*DEBUG* Created entity_relation_task")
                            
                            try:
                                print(f"*DEBUG* Awaiting entity_relation_task...")
                                await entity_relation_task
                                print(f"*DEBUG* Entity relation task completed successfully")
                            except Exception as e:
                                print(f"*ERROR* Entity relation task failed: {e}")
                                raise
                                
                            file_extraction_stage_ok = True
                            print(f"*DEBUG* File extraction stage marked as OK")
    ...
    async def _process_entity_relation_graph(
        ...
        try:
            print(f"*DEBUG* Starting entity extraction for {len(chunk)} chunks")
            print(f"*DEBUG* About to call extract_entities function...")
            ...
            print(f"*DEBUG* Entity extraction completed successfully")
            print(f"*DEBUG* Got {len(chunk_results)} chunk results from entity extraction")
